...
-- =================================================================================
-- PROCEDURE: sp_DeleteOrder
-- DESCRIPTION: Deletes an order and relies on ON DELETE CASCADE to remove related
--              Order_Items, Payments, and Deliveries. Safe to call multiple times.
-- PARAMETERS: p_Order_ID (INT) - The ID of the order to delete.
-- =================================================================================
DROP PROCEDURE IF EXISTS sp_DeleteOrder;
DELIMITER $$
CREATE PROCEDURE sp_DeleteOrder(IN p_Order_ID INT)
BEGIN
    -- Defensive: ensure order exists
    IF (SELECT COUNT(*) FROM Orders WHERE Order_ID = p_Order_ID) = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Order not found.';
    END IF;

    -- Delete the order; cascade will remove related rows where defined
