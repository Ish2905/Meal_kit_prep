...
async function fetchMasterData() {
  try {
    ...
    // also fetch bundles; include customerId when available so bundles respect allergies
    let bundles = [];
    try { bundles = await apiFetch(`/mealkit-bundles${state.currentUser ? `?customerId=${state.currentUser.customerId}` : ''}`); } catch (_) { bundles = []; }
    state.masterData = { ...data, plans, bundles };
  } catch (err) {
...
async function fetchAndRenderRecommendations(customerId) {
  ...
  try {
    ...
    container.innerHTML = `
      <h4 class="text-xl font-semibold mb-4">Available For You</h4>
...
async function fetchAndRenderBrowseByPreference() {
  ...
  async function load(prefId) {
    ...
    try {
      const meals = await apiFetch(`/mealkits/by-preference/${prefId}${state.currentUser ? `?customerId=${state.currentUser.customerId}` : ''}`);
...
function renderOrderForm(availableKits) {
  ...
  container.innerHTML = `
    ...
    <form id="order-form">
      <div class="mb-4">
        <label class="block font-medium mb-1">Select a Plan</label>
        <select name="plan_id" id="order-plan" class="w-full p-2 border rounded-lg">
          ${state.masterData.plans.length ? state.masterData.plans.map(p => `<option value="${p.Plan_ID}">${p.Plan_Name}</option>`).join('') : '<option value="">(no plans)</option>'}
        </select>
      </div>
      <div class="mb-4">
...
// Render bundles in a handy section below recommendations
function renderBundles() {
  const container = document.getElementById('browse-container') || document.getElementById('recommendations-container');
  if (!container) return;
  const bundles = state.masterData.bundles || [];
  if (!bundles.length) return;
  const html = `
    <h4 class="text-xl font-semibold mb-4">Meal Bundles</h4>
    <div class="grid md:grid-cols-3 gap-4">
      ${bundles.map(b => `
        <div class="card bg-white">
          <div>
            <p class="font-bold">${b.name}</p>
            <p class="text-sm text-gray-500">${b.items.length} items</p>
            <ul class="mt-2 text-sm text-gray-600">
              ${b.items.map(i => `<li>${i.Name} — ${i.Cuisine}</li>`).join('')}
            </ul>
          </div>
          <div>
            <button class="btn-primary" data-bundle-id="${b.id}">Order Bundle</button>
          </div>
        </div>`).join('')}
    </div>
  `;
  ...
  container.querySelectorAll('button[data-bundle-id]')?.forEach(btn => btn.addEventListener('click', async () => {
    const id = btn.dataset.bundleId;
    const bundle = bundles.find(x => x.id === id);
    if (!bundle) return;
    // ask for plan selection then place order with all item IDs
    const planId = (state.masterData.plans && state.masterData.plans[0]) ? state.masterData.plans[0].Plan_ID : null;
    if (!planId) return alert('No plans available.');
    const mealkit_ids = bundle.items.map(i => i.MealKit_ID).join(',');
    try {
      const payload = { customer_id: state.currentUser.customerId, plan_id: planId, mealkit_ids, payment_method: 'Credit Card' };
      const res = await apiFetch('/orders', { method: 'POST', body: JSON.stringify(payload) });
      alert(`Order #${res.orderId} placed for bundle ${bundle.name}`);
      fetchAndRenderDashboard(state.currentUser.customerId);
    } catch (e) {
      alert('Failed to place bundle order: ' + e.message);
    }
  }));
}
...
async function handleRegister(e) {
  ...
  try {
    ...
    localStorage.setItem('mkp_user', JSON.stringify(state.currentUser));
    // Refresh master data now that we have a logged-in user (so bundles/preferences are filtered)
...
async function handleLogin(e) {
  ...
  try {
    ...
    localStorage.setItem('mkp_user', JSON.stringify(state.currentUser));
    // Refresh master data after login so bundles and other master-data respect this user
    await fetchMasterData();
    updateUIForLoggedInUser();
    setTimeout(() => { showPage('dashboard'); fetchAndRenderDashboard(state.currentUser.customerId); }, 800);
  } catch (error) {
...
async function fetchAndRenderOrderHistory(customerId) {
  ...
  try {
          ...
          ${rows.map(o => `
            <li class="py-3 flex items-center justify-between">
              ...
              <div class="flex items-center space-x-3">
                <span class="text-sm text-gray-500">${fmtDateTime(o.Order_Date)}</span>
                ${ (state.currentUser && (String(o.Customer_ID) === String(state.currentUser.customerId) || (state.currentUser.email && state.currentUser.email.toLowerCase() === ADMIN_EMAIL.toLowerCase()))) ? `<button class="btn-danger btn-delete-order" data-order-id="${o.Order_ID}">Delete</button>` : '' }
              </div>
            </li>
          `).join('')}
      </ul>
    `;
      ...
      orderHistoryContent.querySelectorAll('button.btn-delete-order')?.forEach(btn => {
        btn.addEventListener('click', async () => {
          const id = btn.dataset.orderId;
          try {
            await apiFetch(`/orders/${id}`, { method: 'DELETE', headers: { 'x-customer-id': state.currentUser?.customerId } });
            // Inform user and refresh
            alert('Order has been deleted');
            fetchAndRenderOrderHistory(state.currentUser.customerId);
          } catch (e) {
...
// Admin: render users list with delete buttons
async function renderAdminUsers() {
  const container = document.getElementById('admin-page');
  if (!container) return;
  let section = container.querySelector('#admin-users');
  if (!section) {
    section = document.createElement('div');
    section.id = 'admin-users';
    section.className = 'bg-white p-8 rounded-xl shadow-lg mt-6';
    section.innerHTML = '<h3 class="text-xl font-semibold mb-4">Users</h3><div id="admin-users-content"><p>Loading...</p></div>';
    container.querySelector('.grid')?.appendChild(section);
  }
  const content = section.querySelector('#admin-users-content');
  try {
    const rows = await apiFetch('/users', { headers: { 'X-Customer-Id': state.currentUser?.customerId } });
    if (!Array.isArray(rows) || rows.length === 0) {
      content.innerHTML = '<p class="text-gray-500 text-sm">No users found.</p>';
      return;
    }
    content.innerHTML = `
      <ul class="divide-y">
        ${rows.map(u => `
          <li class="py-3 flex items-center justify-between">
            <div>
              <p class="font-medium">${u.Name || 'User'} (ID: ${u.Customer_ID})</p>
              <p class="text-sm text-gray-500">${u.Email}</p>
            </div>
            <div>
              <button class="btn-danger btn-delete-user" data-user-id="${u.Customer_ID}">Delete User</button>
            </div>
          </li>
        `).join('')}
      </ul>
    `;
    // Attach delete handlers
    content.querySelectorAll('button.btn-delete-user')?.forEach(btn => {
        btn.addEventListener('click', async () => {
        const id = btn.dataset.userId;
        if (!confirm(`Delete user #${id}? This will remove the user's account.`)) return;
        try {
          await apiFetch(`/users/${id}`, { method: 'DELETE', headers: { 'X-Customer-Id': state.currentUser?.customerId } });
